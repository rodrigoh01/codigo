...
passport.use('local.signup', new LocalStrategy({
    ...
    if (emailBasef.length > 0) {
        ...
        const result = await db.query("INSERT INTO usuarios (nombre,apellido,email,direccion,pass) values('" + newUser.nombre + "','" + newUser.apellido + "','" + newUser.email + "','" + newUser.direccion + "','" + newUser.pass + "')")     

        // solo sirve para recuperar el id del registro ingresado
        const idResult = await db.query("SELECT id FROM usuarios WHERE nombre = '" + newUser.nombre + "' and apellido = '" + newUser.apellido + "' and email = '" + newUser.email + "' and direccion ='" + newUser.direccion + "' and  pass ='" + newUser.pass + "'") 
